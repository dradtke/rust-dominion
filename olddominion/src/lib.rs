//! THIS MODULE IS DEPRECATED, AND HAS BEEN REPLACED. Please use the new one instead.
//!
...
pub fn play(player_list: Vec<Box<Player + Send + Sync>>) {
    ...
    spawn(proc() {
        for _ in range(0u, n) {
            ...
            let future_result = std::task::try_future(proc() {
                ...
                deck.extend(cards::COPPER.create_copies(7).into_iter());
                deck.extend(cards::ESTATE.create_copies(3).into_iter());
        ...
        match receiver.recv().unwrap() {
            ...
            Ok(results) => {
                if results.tie {
                    ...
                    match scores.entry(String::from_str(results.winner)) {
                        Vacant(entry) => entry.set(1),
                        Occupied(mut entry) => { *entry.get_mut() += 1; entry.into_mut() },
                    };
    ...
    if let Some(x) = output_name {
        writeln!(term, "Results saved to {}.", x);
    }
}
...
pub fn play_card(c: Card, params: &[ActionParameter]) -> Result {
    ...
    if let Some(f) = action {
        using_active_card(c, || if let Err(e) = f(params) {
            fail!("{}", e);
        });
    }
    Ok(())
...
fn build_kingdom() -> Vec<Card> {
    ...
    if let Some(cards) = local_kingdom.get() {
        for card in cards.iter().take(KINGDOM_SIZE) {
            kingdom.push(*card);
        }
    }
...
fn log(file: Option<&mut std::io::File>, msg: String) {
    if let Some(Err(e)) = file.map(|f| f.write_line(msg.as_slice())) {
        fail!(e);
    }
}
